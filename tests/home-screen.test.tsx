...
test("[正常] read-only時にcキー押下でカード型へ切り替わること", async () => {
  ...
  // scrollbox のビューポート内に収まるカード境界線を確認（スクロール可能なので全カードが初期表示に収まるとは限らない）
  expect(countRowsWith(frame, "─")).toBeGreaterThanOrEqual(6)
})
...
test("[正常] カード型で複数行メモの全本文行がそれぞれ独立した行に描画されること", async () => {
  ...
  await pressKey(mockInput, "c")
  await renderOnce()
  // メモは逆時系列表示（11:24→10:11→09:14→08:00）のため、08:00複数行メモは初期表示の折り返し直下にある
  // j キーで 2 回スクロールして複数行メモ全行を表示エリアに収める
