...
test("[正常] read-onlyでrキー押下後にStatusBarにReloaded HH:MMが表示されること", async () => {
  const { mockInput, captureCharFrame, renderOnce } = await renderApp()
  await renderOnce()
  await pressKey(mockInput, "r")
  await renderOnce()
  const frame = captureCharFrame()
...
test("[正常] RキーでStatusBarのintervalラベルが切り替わること", async () => {
  const { mockInput, captureCharFrame, renderOnce } = await renderApp({ reloadInterval: 60 })
  await renderOnce()
  const frame0 = captureCharFrame()
  expect(frame0).toContain("[60s]")

  await pressKey(mockInput, "r", { shift: true })
  await renderOnce()
  const frame1 = captureCharFrame()
  expect(frame1).toContain("[30s]")
})

test("[正常] reloadInterval=offのとき StatusBarにinterval[off]が表示されること", async () => {
  const { captureCharFrame, renderOnce } = await renderApp({ reloadInterval: "off" })
  await renderOnce()
  const frame = captureCharFrame()

  expect(frame).toContain("interval[off]")
})
